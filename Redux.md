...
```
dispatch(increment())
→ action created { type: 'counter/increment' }
→ store receives action
→ counter reducer runs
→ state.counter.value updates
...
- `incrementByAmount(5)` → `5` becomes `action.payload`
- Reducer reads `action.payload`
- State updates based on external data

### 🔁 Step 7 — Redux Data Flow

- Redux always follows **ONE-WAY data flow:**

```
UI → dispatch(action)
Action created (RTK auto) → reducer runs → store update → UI re-renders
```
...
### ⚡ Step 8 — Async Redux with createAsyncThunk
